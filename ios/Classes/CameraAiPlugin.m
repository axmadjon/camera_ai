...
- initWithPath:(NSString *)path
 ...
 motionManager:(CMMotionManager *)motionManager
cameraPosition:(AVCaptureDevicePosition)cameraPosition
faceDetector:(MyFaceDetector *)faceDetector {
    self = [super init];
...
@property(readonly, nonatomic) MyFaceDetector* faceDetector;
...
- (instancetype)initWithCameraName:(NSString *)cameraName
    ...
    _faceDetector = [[MyFaceDetector alloc] init];
...
didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer
    ...
    if (CMSampleBufferDataIsReady(sampleBuffer)) {
        if (![_faceDetector detection]) {
            [_faceDetector predictWithSampleBuffer: sampleBuffer
                                    cameraPosition: _captureDevice.position];
        }
    } else {
        _eventSink(@{
    ...
    if (_isStreamingImages) {
        if (_imageStreamHandler.eventSink) {
            ...
            NSString *result = [_faceDetector getResultToString];
            ...
            if ([_faceDetector isOnlyBase64]) {
                imageBuffer[@"frame"] = [_faceDetector getCurrentFrameToBase64];
            }
            imageBuffer[@"result"] = result;
...
- (void)startImageStreamWithMessenger:(NSObject<FlutterBinaryMessenger> *)messenger {
    if (!_isStreamingImages) {
        ...
        [FlutterEventChannel eventChannelWithName:@"plugins.datalab.uz/camera/imageStream"
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    ...
    [FlutterMethodChannel methodChannelWithName:@"plugins.datalab.uz/camera"
                                binaryMessenger:[registrar messenger]];
    CameraAiPlugin *instance = [[CameraAiPlugin alloc] initWithRegistry:[registrar textures]
...
- (void)handleMethodCallAsync:(FlutterMethodCall *)call result:(FlutterResult)result {
    if ([@"availableCameras" isEqualToString:call.method]) {
        ...
        if (error) {
            ...
            FlutterEventChannel *eventChannel = [FlutterEventChannel
                                                 eventChannelWithName:[NSString
                                                                       stringWithFormat:@"datalab.uz/cameraPlugin/cameraEvents%lld",
                                                                       textureId]
        ...
        bool analyzeMaxFace = call.arguments[@"face_analyze"];
        bool onlyBase64 = call.arguments[@"only_base64"];
        [[_camera faceDetector] setWithAnalyzeMaxFace:analyzeMaxFace onlyBase64:onlyBase64];
        ...
        result(nil);
    } else if ([@"resetFaceId" isEqualToString:call.method]) {
        NSString *ids = call.arguments[@"ids"];
        [[_camera faceDetector] resetFaceIdWithIds: ids];
        result(nil);
    } else {
